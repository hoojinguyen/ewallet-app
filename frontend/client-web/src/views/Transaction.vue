<template>
  <div>
    <div class="row m-0">
      <div class="col-12 main-content">
        <div class="text-right mb-3">
          ...
          >Thêm mới giao dịch</el-button>
        </div>
        <div class="transaction-display">
          <el-tabs v-model="activeMonth" @tab-click="handleClick" :stretch="true">
            <el-tab-pane name="lastMonth" label="Tháng trước">
              <p v-if="dataLastMonth.length == 0">Chưa có giao dịch nào!</p>
              <list-item-transaction
                v-else
                class="item-transaction"
                v-for="(item,index) in dataLastMonth"
                :key="index"
            ...
            <el-tab-pane name="currentMonth" label="Tháng này">
              <p v-if="dataCurrentMonth.length == 0">Chưa có giao dịch nào!</p>
              <list-item-transaction
                v-else
                class="item-transaction"
                v-for="(item,index) in dataCurrentMonth"
                :key="index"
                :dataTransaction="item.listTransaction"
                :detailTime="item.detailTime"
                v-on:openDetailTransactionEvent="openDetailTransaction"
            ...
            <el-tab-pane name="featureMont" label="Tương lai">Chưa có giao dịch nào!</el-tab-pane>
          </el-tabs>
        </div>
      </div>
    </div>
    ...
    <!-- Modal Add -->
    <el-dialog title="Thêm mới giao dịch" :visible.sync="dialogFormVisible" width="50%">
      <el-form :model="formAddNew" label-position="right" label-width="100px">
        <div class="row">
          <div class="col-4">
            <el-form-item label="Ví: " :label-width="formLabelWidth">
              <el-select v-model="formAddNew.walletId" placeholder="Chọn ví" :disabled="true">
                <el-option
                  :label="wallet.displayName"
                  :value="wallet.id"
                  v-for="wallet in walletsList"
          ...
          <div class="col-8">
            <el-form-item label="Chọn nhóm:" :label-width="formLabelWidth">
              <el-dropdown
                trigger="click"
                class="col-8 float-left"
                v-on:command="changeCategoryActionAdd"
                @visible-change="onToggleListCategory()"
                disabled="true"
              >
                <img :src="imageCategoryCurrent" alt="icon" style="width:40px; height:40px" />
                <!-- <span style="font-size:1rem" class="ml-2">{{nameCategoryCurrent}}</span> -->
                <span
                  style="font-size:1rem"
                  class="ml-2 text-primary font-weight-bold"
                  v-if="isMoneyAddCurrent"
                >{{nameCategoryCurrent}}</span>
                <span
                  style="font-size:1rem"
                  class="ml-2 text-danger font-weight-bold"
                  v-else
                ...
                <el-dropdown-menu slot="dropdown">
                  ...
                  <el-dropdown-item
                    ...
                    :command="{idCategory: debt.id,nameCategory: debt.displayName, image: debt.image, isMoneyAdd:debt.isMoneyAdd}"
                  >
                    <img :src="debt.image" alt="icon" style="width:20px; height:20px" class="mr-2" />
                    <span>{{debt.displayName}}</span>
                  </el-dropdown-item>
                </el-dropdown-menu>
              </el-dropdown>
        ...
        <div class="row">
          ...
          <div class="col-4">
            <el-form-item label="Với" :label-width="formLabelWidth">
              <el-input type="text" placeholder="Với một ai đó" v-model="formAddNew.contact"></el-input>
            </el-form-item>
          </div>
          <div class="col-4">
            <el-form-item label="Ngày" :label-width="formLabelWidth">
        ...
        <div class="col-row">
          <el-form-item label="Ghi chú: " :label-width="formLabelWidth">
            <el-input type="textarea" v-model="formAddNew.description"></el-input>
          </el-form-item>
        </div>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button @click="dialogFormVisible = false">Hủy</el-button>
        <el-button type="success" v-on:click="addNewTransaction">Lưu</el-button>
      </span>
    </el-dialog>
    <!-- Modal Add -->
  </div>
...
export default {
  ...
  created() {},
  mounted() {
    this.$store.dispatch("retrieveTransactions", this.walletIdTest2);
  ...
  computed: {
    ...mapGetters({
      dataCurrentMonth: "getTransactionCurrentMonth",
      dataLastMonth: "getTransactionLastMonth",
  ...
  methods: {
    ...mapActions({
      actionAddNewTransaction: "actionAddNewTransaction",
    ...
    addNewTransaction() {
      if (!this.formAddNew.categoryDetail) {
        this.$notify.error({
          title: "Lỗi !!!",
        ...
        return;
      } else if (this.formAddNew.money == "" || this.formAddNew.money < 0) {
        this.$notify.error({
          title: "Lỗi !!!",
      ...
      this.actionAddNewTransaction(this.formAddNew);
      ...
      this.formAddNew = this.formEmptyDefault;
    },
    updateTransaction() {
